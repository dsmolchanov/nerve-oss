...
## Embedded migration owner review correction — 2026-09-10

Explicitly include `cmd/neuralmaild/main.go`, both command packages' schema-window tests and `internal/release/embedded_migration_executable_test.go`. All six embedded migration commands across runtime and administrative CLI must reject successor/malformed compiled windows before opening a database. Guard each shared command wrapper and the Core/Cloud helpers; combined migrations delegate to those guarded helpers. Audit all command-level migration callers, leaving only nerve-migrate as the successor migration owner. The real-binary regression covers both executables and all three embedded commands, successor and malformed windows, plus historical-build negative controls against an invalid DB DSN.

Owner-fence validation: the new real-binary test first reproduced nerve-runtime reaching the invalid DB instead of refusing. After the fix, race tests for startup/release/both embedded CLIs/nerve-migrate, targeted vet, the final isolated executable matrix, and the existing quiescence executable suite PASS. Historical negative controls still reach their intentionally invalid database; successor and malformed-window cases refuse before database access.

## Extractable runtime manifest — 2026-09-10
