...
CREATE UNIQUE INDEX IF NOT EXISTS idx_org_domains_verified ON org_domains(lower(domain))
  WHERE status IN ('verified_dns', 'provisioning', 'active');
-- Allow multiple pending claims for the same domain (they expire)
CREATE INDEX IF NOT EXISTS idx_org_domains_domain ON org_domains(lower(domain));
CREATE INDEX IF NOT EXISTS idx_org_domains_org ON org_domains(org_id);
CREATE INDEX IF NOT EXISTS idx_org_domains_expires ON org_domains(expires_at) WHERE status = 'pending';
...
ALTER TABLE inboxes ADD COLUMN IF NOT EXISTS org_domain_id uuid REFERENCES org_domains(id);
